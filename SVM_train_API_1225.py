...
command_cod = "python MLGame.py -f 60 -1 -r pingpong -i number_11_P1_API.py number_11_P2_API.py"
...
first_run = 0
#-----------------------------------------initial data
Initial_data_x_P1_L = np.array( [0,   420, 7, -7, 7,  80, 80, 80,   0, 80,   -440] )
Initial_data_x_P1_R = np.array( [190, 420, 7,  7, 7,  80, 80, -110, 0, -110, -440] )

Initial_data_x_P2_L = np.array( [0,   50,   7, -7, -7, 80, 80, 80,   420, 80,    0] )
Initial_data_x_P2_R = np.array( [190, 50,   7,  7, -7, 80, 80, -110, 420, -110,  0] )
...
#-----------------------------------------data fix and prepare
    ...
    #------------------ make feature
    last_speed = Ball_speed[-1][0]
    parameter_frame = int(last_speed * 5 / 7.5 + 0.5)

    if who_lose_need_fix == 1 : 
        last_ball_y = Ball[-1][1]
        catch_frame = int( (last_ball_y - 420) / Ball_speed[-1]  + 1.5 )
        ...
        train_starting_point = fix_quantity + catch_frame + parameter_frame

        if last_plat_x >= final_ball_x :
            P1_y = [-1] * (train_starting_point - catch_frame)
        else : 
            P1_y = [1] * (train_starting_point - catch_frame)
        P2_y = [0] * (train_starting_point - catch_frame)

    elif who_lose_need_fix == 2 :
        last_ball_y = Ball[-1][1]
        catch_frame = int( (50-last_ball_y) / Ball_speed[-1]  + 1.5  )
        ...
        if last_plat_x >= final_ball_x :
            P2_y = [-1] * (train_starting_point - catch_frame)
        else : 
            P2_y = [1] * (train_starting_point -catch_frame)
        P1_y = [0] * (train_starting_point - catch_frame)

    fix_data_x = original_data_x[ -train_starting_point : -catch_frame]
